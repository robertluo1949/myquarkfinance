...
C-02-02-REALNAME
    ...
    \    get_identifyNO
    \    log    ${temp_id_no}
    \    ${ID}=    replace variables    ${temp_id_no}
    \    set global variable    ${ID}
    \    set to dictionary    ${bizContent}    identificationNo    ${ID}
    ...
    \    Continue For Loop If    '${code}'== 'CHECKED'
    \    PASS Execution If    '${code}'== 'OK'    SET TAGS    ${ID} is a correct ID number.
    \    PASS Execution If    '${code}'== 'OK'    exit for loop
    \    Comment    run keyword if    '${code}'== 'USED'    FAIL    SET TAGS    ${mobile} is a checked mobile number.you need a new mobile number.
    should be equal    '${code}'    'OK'    ${code}${resMsg}

C-02-03-query_db
    [Documentation]    验证该用户的ID信息是否插入数据库，app_user表
    Comment    Set Environment Variable    NLS_LANG    SIMPLIFIED_CHINESE_CHINA_UTF8    ##设置oracle编码格式
    Import Resource    ${CURDIR}\\resourcefile.robot
    LOG    ${ID}
    log    ${mobile}
