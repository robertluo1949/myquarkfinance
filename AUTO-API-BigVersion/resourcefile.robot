...
getTOKEN
    ...
    ...    client_id=APP    Accept-Encoding=    Accept=application/json, text/plain, */*    User-Agent=Mozilla/5.0 (Linux; Android 5.1.1; vivo X6S A Build/LMY47V) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/39.0.0.0 Mobile Safari/537.36    Content-Length=545    Host=${ipaddress}
...
get_VOUCHERCODE
    [Arguments]    ${testdb}    ${batchid}    ${status}
    [Documentation]    Name: Get_VOUCHERCODE
    ...    Source: resource file \ <test library>
    ...    Arguments: [ ${testdb} | ${batch_id} |{status}]
    ...    获取券码ID
    import resource    ${CURDIR}\\resourcefile.robot
    QUERYDB    ${testdb}    select id,batch_id,code from VOUCHER_CODE_DETAIL where BATCH_ID=${batch_id} AND STATUS=${status} and rownum = 1 order by ID
    run keyword if    ${result} == None    set tags    you should update VOUCHER_batchid.
    ${VOUCHER_CODE}    set variable    ${result[0][2]}
    set suite variable    ${VOUCHER_CODE}
...
get_identifyNO
    ...
    RUN PROCESS    python    ${CURDIR}\\libs\\put_identifyno.py    alias=get_id
    ${result}    get process result    get_id
    ${temp_result}    convert to string    ${result.stdout}    ##通过stdout属性获取result
    ${temp_id_no}    set variable    ${temp_result[-18:]}
    set suite variable    ${temp_id_no}
